...
// visitors selectors
var visitorSelector = "li.artdeco-list__item"
var visitorNameSelector = ".artdeco-entity-lockup__title"
var visitorConnDegreeSelector = ".artdeco-entity-lockup__badge"
var visitorSubtitleSelector = ".artdeco-entity-lockup__subtitle"
var visitorMetadataSelector = ".artdeco-entity-lockup__metadata"
...
var visitorsDetails = document.querySelectorAll(visitorSelector) // to get visitor info
...
function getUrl(curItem, isJob = false, isJobCompany = false) {
    ...
    return curItem.querySelector(urlSelector) ? curItem.querySelector(urlSelector).href : ""
}

function getVisitors() {
    for (let i = 0; i < visitorsDetails.length; i++) {
        let curVisitor = visitorsDetails[i]

        let curUrl = getUrl(curVisitor)

        if (curUrl.includes("/in/")) {
            let curVisitorTitle = curVisitor.querySelector(visitorNameSelector).innerText.split("\n")
            let curVisitorMetadata = curVisitor.querySelector(visitorMetadataSelector) ? curVisitor.querySelector(visitorMetadataSelector).innerText.split("\n") : ""
            let visitorMetadata = []

            let visitorName = curVisitorTitle[0]
            let visitorProfileUrl = curUrl
            let visitorJobTitle = curVisitor.querySelector(visitorSubtitleSelector) ? curVisitor.querySelector(visitorSubtitleSelector).innerText : ""
            let visitorConnDegree = curVisitor.querySelector(visitorConnDegreeSelector) ? curVisitor.querySelector(visitorConnDegreeSelector).innerText : ""
            let visitorRequestStatus = curVisitor.querySelector(entityButtonActionSelector) ? curVisitor.querySelector(entityButtonActionSelector).innerText : ""
            visitorMetadata.push(curVisitorMetadata[0])
            if (curVisitorMetadata.length > 1)
            {
                visitorMetadata.push(curVisitorMetadata[1])
            }

            let curVisitorObj = new Visitor(visitorName, visitorProfileUrl, visitorJobTitle, visitorConnDegree, visitorRequestStatus, visitorMetadata)

            if (!recruiters.some(x => JSON.stringify(x) === JSON.stringify(curVisitorObj))) recruiters.push(curVisitorObj)
        }
    }
}
